import { BOT_TARGET_WIN_RATES } from './constants.ts';
import { getPlayableShapes, isWhot } from './cards.ts';
import { getPickCountForCard } from './effects.ts';
import { getEffectiveShape, getTopCard } from './moves.ts';
import { getLegalMoves } from './state.ts';
import type {
  BotConfig,
  BotDecisionContext,
  Card,
...
export function getBotConfigForStake(stakeLevel: StakeLevel): BotConfig {
  return {
    ...DEFAULT_CONFIG,
    targetWinRate: BOT_TARGET_WIN_RATES[stakeLevel] ?? 0.55,
  };
}
