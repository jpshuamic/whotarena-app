/**
 * Server-authoritative move handler (Section 11)
 * Validates moves, updates game state, broadcasts via Realtime.
 * Bot decisions run here — never on client.
 */

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { applyMove, calculatePotKobo } from '../_shared/state.ts';
import type { GameState, MoveInput, PlayerSlot } from '../_shared/types.ts';
...
serve(async (req) => {
  ...
  const authUser = await getUserFromAuth(authorization);
  if (!authUser?.id) {
  ...
  const move = payload.move as MoveInput | undefined;

  if (!gameId || !move) {
    return createJsonResponse({ error: 'Missing gameId or move' }, 400);
  }

  const safeMove = { ...move, playerId: authUser.id } as MoveInput;

  try {
    const gameRows = await fetchSupabase(
    ...
    const [roomRows, roomPlayers, handRows, moveRows] = await Promise.all([
      fetchSupabase(`/rest/v1/rooms?select=entry_fee,stake_level&id=eq.${encodeURIComponent(game.room_id)}`),
      fetchSupabase(
    ...
    if (!playerIds.includes(authUser.id)) {
    ...
    const nextState = applyMove(state, safeMove);
    ...
    const updateGameResponse = await fetchSupabase(
    ...
    settlementOperations.push(...handUpdates);

    const movePayload = {
      game_id: gameId,
      player_id: authUser.id,
      move_type: move.type,
      card_played: move.type === 'play' ? state.hands[authUser.id]?.find((card: any) => card.id === move.cardId) ?? null : null,
      whot_call: move.type === 'play' ? (move.whotCall ?? null) : null,
      game_state: nextState,
    };

    settlementOperations.push(
      fetchSupabase('/rest/v1/game_moves', {
        method: 'POST',
        headers: {
          ...DEFAULT_HEADERS,
          Prefer: 'return=representation',
    ...
    await Promise.all(settlementOperations);

    return createJsonResponse({ ok: true, game: updateGameResponse });
  } catch (error) {
